...
**前置条件**: [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) (`npm install -g @anthropic-ai/claude-code`) + Python 3 + [Git](https://git-scm.com/download/win)

> **Windows 用户**：
>
> 1. 首次运行配置脚本（只需一次）：
>    ```powershell
>    .\claude-auto-loop\setup-windows.ps1
>    ```
>    这会自动配置 PowerShell，使 `bash` 命令直接调用 Git Bash。
>
> 2. 之后即可像 Mac/Linux 用户一样直接运行 `.sh` 脚本：
>    ```bash
>    bash claude-auto-loop/run.sh "你的需求"
>    ```
>
> *（备选方案：CMD 用户或不想配置 Profile 的用户，仍可使用 `claude-auto-loop\loop.bat run`）*
...
更新采用**排除策略**：保留项目运行时数据（`config.env`、`tasks.json`、`progress.txt`、`project_profile.json`、`init.sh`、`logs/` 等），覆盖所有核心文件（`CLAUDE.md`、`run.sh`、`setup.sh`、`validate.sh`、`hooks/` 等）。新增的核心文件会自动同步，无需手动维护文件列表。
...
| `--add "指令"` | 任务追加模式，根据指令生成新任务到 tasks.json（不执行编码） | — |
...
bash claude-auto-loop/run.sh --add "添加用户头像上传功能" # 追加新任务
```
...
当所有任务已完成时，`--view` 会自动跳过 6 步工作流（不运行 init.sh、不恢复上下文等），直接进入对话模式。

**模式对比**：

| | 自动化模式 | 观测模式 (`--view`) | 任务追加 (`--add`) |
|---|---|---|---|
| 命令 | `bash run.sh "需求"` | `bash run.sh --view` | `bash run.sh --add "指令"` |
| 可见性 | 仅最终文本 + 进度指示器 | 实时工具调用、文件 diff、thinking | 仅最终文本 |
| 退出 | 自动退出并循环 | 手动退出（`Ctrl+C` 或 `/exit`） | 自动退出 |
| 执行内容 | 完整 6 步流程 | 6 步流程（任务全完成时跳过） | 仅更新 tasks.json |
| 适合 | 无人值守、批量执行 | 调试 prompt、观察行为、验证单个任务 | 所有任务完成后追加新需求 |

### 任务追加模式（所有任务完成后追加需求）

当所有任务都已 `done`，编码循环会立即退出。此时如果有新需求，可用 `--add` 模式生成新的 `pending` 任务：

```bash
bash claude-auto-loop/run.sh --add "添加用户头像上传功能，支持裁剪和压缩"
bash claude-auto-loop/run.sh      # 继续编码循环，执行新任务
```

`--add` 模式是轻量级 session：不执行 6 步流程、不运行 init.sh、不实现代码。Agent 仅读取 `tasks.json` 和 `project_profile.json`，根据指令追加新任务，然后 git commit。
...
| 脚本 | 何时调用 | 说明 |
|------|----------|------|
| **check_prerequisites** | run.sh 启动时自动执行 | 检查 claude CLI、Python 3（python3 或 python）、CLAUDE.md、SCAN_PROTOCOL.md、validate.sh 是否存在；无 config.env 时会提示可运行 setup.sh |
...
**实时活动日志**：hook 在每次工具调用时写入 `.activity_log`，记录工具名和操作摘要（如 `Read backend/app/main.py`、`Bash npm install`）。进度提示会读取最新一条活动，显示为：
- `AI 编码中 · 步骤4-增量实现 · Write backend/app/api.py`
...
编码循环中，Agent 的第二步是「环境与健康检查」（运行 init.sh 安装依赖、启动服务）。为避免每个 session 都重复执行：

- **连续成功时跳过**：当上一个 session 成功完成，run.sh 会注入提示告知 Agent 环境已就绪，可跳过 init.sh，仅快速 curl 验证服务存活
- **首次或失败后恢复**：第一个 session 或失败回滚后，Agent 执行完整的环境检查
- **安全阀**：如果当前任务涉及新依赖变更（如修改了 package.json），Agent 仍会自行决定运行 init.sh
...
| `--add` 追加 | `bash claude-auto-loop/run.sh --add "新功能描述"` | 所有任务完成后追加新需求，Agent 自动拆解为 tasks.json 中的新任务 |
...
| 进度提示 | 无 | PreToolUse hook 精准切换「思考中」→「AI 编码中」+ 实时活动日志 |
...
| 环境优化 | 每次都全量初始化 | 连续成功时跳过 init.sh，仅快速健康检查 |
| 任务追加 | 无 | `--add` 模式：所有任务完成后轻量追加新任务 |
...
| `hooks/phase-signal.py` | PreToolUse hook：写入 `.phase`（进度提示切换）、`.phase_step`（步骤推断）、`.activity_log`（实时活动日志） |
| `hooks-settings.json` | Claude Code hooks 配置，run.sh 通过 `--settings` 加载 |
| `update.sh` | 从 upstream 拉取最新代码（排除策略：保留项目运行时数据，同步所有核心文件） |
| `ARCHITECTURE.md` | 工具架构文档：系统概览、Mermaid 时序图、文件职责说明（供 AI Agent 快速理解工具设计） |
| `README.md` | 本文件 |
...
| `sync_state.json` | 需求同步状态（`last_requirements_hash` 等，条件触发时创建） |
| `.phase` | 进度状态文件（thinking/coding），由 hook 写入，已 gitignore |
| `.phase_step` | 当前推断的 6 步流程步骤，由 hook 写入，已 gitignore |
| `.activity_log` | 实时活动日志（工具调用摘要），由 hook 写入，已 gitignore |
